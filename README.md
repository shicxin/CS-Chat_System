...
3.   相互独立的进程之间文件描述符的发送与接收（必须使用`unix`域socket）
...
4.   bind() error!
     ...
     sequenceDiagram
         ...
         activate 主机A
         Note left of 主机A: time_wait<br/>默认三分钟
     	deactivate 主机A
     ```
     ...
     1.   笨办法：服务器不监听固定端口；
     2.   客户端主动断开，由于客户端的端口是随机的，time_wait对程序整体没有影响；
     3.   服务器根据套接字可选项将time_wait置零；
     4.   服务器通过套接字可选项将time_wait套接字分配给一个监听新端口的套接字避免这个问题。
